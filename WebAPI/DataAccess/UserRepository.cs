 ...
 public User GetUserbyUserName(string username)
  {
      ...
      SqlDataReader reader = command.ExecuteReader();
        ...
        // {
          while(reader.Read())
         {
         foundUser = new User((int)reader[0], (string)reader[1], (string)reader[2], (string)reader[3]);
         }
 ...
 public User AddUser(User newUserToRegister)
    {
      User UserSet = new User();  
        ...
        SqlConnection conn = _connectionFactory.GetConnection();
        SqlCommand command = new SqlCommand("insert into project.users (username, password, user_role) values (@username, @password, @user_role)", conn);
